...
Effect* Sequence::GetSelectedGizmo() {
  if (!amber::app_ctx) {
    qWarning() << "Sequence::GetSelectedGizmo: app_ctx is null";
    return nullptr;
  }
      ...
      if (!c->effects.isEmpty()) {
        ...
        for (const auto& effect : c->effects) {
          ...
          if (e->are_gizmos_enabled()) {
            ...
            if (amber::app_ctx->isEffectSelected(e)) {
              gizmo_ptr = e;
              break;
            }
          }
...
bool Sequence::IsClipSelected(int clip_index, bool containing) {
  Clip* c = clips.at(clip_index).get();
  if (!c) {
    qWarning() << "Sequence::IsClipSelected: clip at index" << clip_index << "is null";
    return false;
  }
  return IsClipSelected(c, containing);
}

bool Sequence::IsClipSelected(Clip* clip, bool containing) {
  if (!clip) {
    qWarning() << "Sequence::IsClipSelected: clip is null";
    return false;
  }
...
bool Sequence::IsTransitionSelected(Transition* t) {
  ...
  if (!c) {
    qWarning() << "Sequence::IsTransitionSelected: parent_clip is null";
    return false;
  }

  int transition_track = c->track();
...
SequencePtr amber::ActiveSequence = nullptr;
