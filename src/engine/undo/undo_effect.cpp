...
void SetEffectEnabled::doUndo() {
  if (!effect_) {
    qWarning() << "SetEffectEnabled::doUndo: effect is null";
    return;
  }
...
void SetEffectEnabled::doRedo() {
  if (!effect_) {
    qWarning() << "SetEffectEnabled::doRedo: effect is null";
    return;
  }
...
void AddEffectCommand::doRedo() {
  if (!clip) {
    qWarning() << "AddEffectCommand::doRedo: clip is null";
    return;
  }
  if (ref == nullptr) {
...
void EffectDeleteCommand::doUndo() {
  if (!parent_clip_) {
    qWarning() << "EffectDeleteCommand::doUndo: parent_clip is null";
    return;
  }
  parent_clip_->effects.insert(index_, deleted_obj_);

  if (amber::app_ctx) amber::app_ctx->refreshEffectControls();
}

void EffectDeleteCommand::doRedo() {
  if (!effect_) {
    qWarning() << "EffectDeleteCommand::doRedo: effect is null";
    return;
  }
  parent_clip_ = effect_->parent_clip;
  if (!parent_clip_) {
    qWarning() << "EffectDeleteCommand::doRedo: parent_clip is null";
    return;
  }
...
void MoveEffectCommand::doUndo() {
  if (!clip) {
    qWarning() << "MoveEffectCommand::doUndo: clip is null";
    return;
  }
...
void MoveEffectCommand::doRedo() {
  if (!clip) {
    qWarning() << "MoveEffectCommand::doRedo: clip is null";
    return;
  }
...
void SetEffectData::doUndo() {
  if (!effect) {
    qWarning() << "SetEffectData::doUndo: effect is null";
    return;
  }
...
void SetEffectData::doRedo() {
  if (!effect) {
    qWarning() << "SetEffectData::doRedo: effect is null";
    return;
  }
...
void SetIsKeyframing::doUndo() {
  if (!row) {
    qWarning() << "SetIsKeyframing::doUndo: row is null";
    return;
  }
...
void SetIsKeyframing::doRedo() {
  if (!row) {
    qWarning() << "SetIsKeyframing::doRedo: row is null";
    return;
  }
  row->SetKeyframingInternal(b);
}
...
void KeyframeDelete::doUndo() {
  if (!field) {
    qWarning() << "KeyframeDelete::doUndo: field is null";
    return;
  }
...
void KeyframeDelete::doRedo() {
  if (!field) {
    qWarning() << "KeyframeDelete::doRedo: field is null";
    return;
  }
...
void KeyframeAdd::doUndo() {
  if (!field) {
    qWarning() << "KeyframeAdd::doUndo: field is null";
    return;
  }
  field->keyframes.removeAt(index);
  done = false;
...
void KeyframeAdd::doRedo() {
  if (!field) {
    qWarning() << "KeyframeAdd::doRedo: field is null";
    return;
  }
  if (!done) {
...
{
  if (!field_) {
    qWarning() << "KeyframeDataChange::SetNewKeyframes: field is null";
    return;
  }
...
{
  if (!field_) {
    qWarning() << "KeyframeDataChange::doUndo: field is null";
    return;
  }
...
{
  if (!field_) {
    qWarning() << "KeyframeDataChange::doRedo: field is null";
    return;
  }
  if (!done_) {
